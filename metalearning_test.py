    ...
    for _ in range(epochs):
        ...
        for i, t in enumerate(random.sample(dataset, len(dataset))):
            ...
            with tf.GradientTape() as test_tape:
                ...
                model_copy.compile(optimizer=keras.optimizers.SGD(learning_rate=0.01),
                                   loss='mse')
                # model_copy.fit(x, y, epochs=1, batch_size=len(x))
                model_copy.train_on_batch(x, y)
                # print(train_loss)
                ...
                test_loss = loss_function(model_copy.call(x), y)
            # Step 8
...
maml = layers.Dense(40)(inputs)
maml = layers.ReLU()(maml)
maml = layers.Dense(40)(maml)
maml = layers.ReLU()(maml)
outputs = layers.Dense(1)(maml)

maml = keras.Model(inputs, outputs)

# maml.add(keras.layers.Dense(40))
# maml.add(layers.ReLU())
# maml.add(layers.Dense(40))
# maml.add(layers.ReLU())
# maml.add(layers.Dense(1))
